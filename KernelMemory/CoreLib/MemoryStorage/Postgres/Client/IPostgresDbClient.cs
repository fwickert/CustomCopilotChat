...
public interface IPostgresDbClient
{
    ...
    /// <param name="tags">The tags of the entry.</param>
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for cancellation requests. The default is <see cref="CancellationToken.None"/>.</param>
    /// <returns></returns>
    Task UpsertAsync(string tableName, string key, string? payload, string? tags, Vector? embedding, DateTime? timestamp, CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets the nearest matches to the <see cref="Vector"/>.
    /// </summary>
    /// <param name="tableName">The name assigned to a table of entries.</param>
    /// <param name="tags">The tags of the entries to search.</param>
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for cancellation requests. The default is <see cref="CancellationToken.None"/>.</param>
    /// <returns>An asynchronous stream of <see cref="PostgresMemoryRecord"/> objects that the nearest matches to the <see cref="Vector"/>.</returns>
    IAsyncEnumerable<(PostgresMemoryRecord, double)> GetNearestMatchesAsync(string tableName, ICollection<MemoryFilter>? filters, Vector embedding, int limit, double minRelevanceScore = 0, bool withEmbeddings = false, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for cancellation requests. The default is <see cref="CancellationToken.None"/>.</param>
    /// <returns></returns>
    Task<PostgresMemoryRecord?> ReadAsync(string tableName, string key, bool withEmbeddings = false, CancellationToken cancellationToken = default);
    ...
    IAsyncEnumerable<PostgresMemoryRecord> ReadBatchAsync(string tableName, IEnumerable<string> keys, bool withEmbeddings = false, CancellationToken cancellationToken = default);
