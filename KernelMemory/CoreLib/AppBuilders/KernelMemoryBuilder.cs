    ...
    public KernelMemoryBuilder FromConfiguration(KernelMemoryConfig config, IConfiguration servicesConfiguration)
    {
        ...
        switch (config.Retrieval.VectorDbType)
        {
            ...
            case string x when x.Equals("Postgres", StringComparison.OrdinalIgnoreCase):
               this._memoryServiceCollection.AddPostgresAsVectorDb(this.GetServiceConfig<PostgresConfig>(config, "Postgres"));
                this._hostServiceCollection?.AddPostgresAsVectorDb(this.GetServiceConfig<PostgresConfig>(config, "Postgres"));
                break;
            default:
